...
//Get The ABI
           ...
           if(netWorkId===localNetWork && account != null ){
           ...
           if(nftMarketWorkObject && nftMarketWorkObject&&TokenMarketWorkObject){
            ...
            const deployedMarketContractWithWallet = await new localWeb3.eth.Contract(markrtAbi,marketAddress);
            ...
            if(account){
              const myBalance = await localWeb3.eth.getBalance(account)
              const convertBalance = await  localWeb3.utils.fromWei(myBalance,"ether")
...
const buyNFT = async (nftItem)=>{
    ...
    console.log("marketContract from NFT BUY",marketContract)
    console.log("tokenContract from NFT BUY ",tokenContract)
      ...
      if(account && tokenContract){
        if(nftItem.isResell){
          ...
          try{
              ...
              let sellerMoney = (priceinNumber - creatorCommission).toFixed(18);
...
////===================>>>>>>>>>>>>>
  ...
  return(
      ...
      <br/>
      {
        isLoading?<Preloader Title={"loading Top 4 "} /> :  <section className="features section-padding-0-100 ">

        <div className="container pt-5">
            <InfoComponent
              titleSm='Dicover All New Items'
            />
        
            <div className="row align-items-center">
                { unsoldItems.map((item , i) => (
          <ListedItemsItem >{{buttonTitle:buyButtonTitle,itemId:item.itemId,image:item.image,title:item.name,price:item.price,creator:item.creator,buy:async()=>{
            buyNFT(item);
          }}}</ListedItemsItem>           
                ))}
